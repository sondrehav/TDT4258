...
void drawBoard ();
void enterGameLoop ();
void drawNumber (uint32_t x, uint32_t y, color numColor, uint16_t num,
		 FILE * framebuffer);
...
static void
drawRectangle (uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, color col)
{
...
void
enterGame (FILE * framebufferDriver)
{
...
void
onKeyDown (uint32_t key)
{
  if ((key & 2) == 2)
    leftPlayer.movingUp = true;	// SW2
  else if ((key & 8) == 8)
    leftPlayer.movingDown = true;	// SW4
  else if ((key & 32) == 32)
    rightPlayer.movingUp = true;	// SW6
  else if ((key & 128) == 128)
    rightPlayer.movingDown = true;	// SW8
}

// Check which button was realeased
void
onKeyUp (uint32_t key)
{
  if ((key & 2) == 2)
    leftPlayer.movingUp = false;	// SW2
  else if ((key & 8) == 8)
    leftPlayer.movingDown = false;	// SW4
  else if ((key & 32) == 32)
    rightPlayer.movingUp = false;	// SW6
  else if ((key & 128) == 128)
...
void
drawPlayer (PlayerState * player, color colorIn)
{
  uint32_t x0;
  if (player->leftBoardPosition)
    x0 = PLAYER_SCREEN_OFFSET;
  else
    x0 = V_SCREEN_WIDTH - PLAYER_SCREEN_OFFSET - 1;
  uint32_t x1 = x0 + 1;
...
void
clearMovement (PlayerState * player, color colorIn)
{
  uint32_t x0;
  if (player->leftBoardPosition)
    x0 = PLAYER_SCREEN_OFFSET;
...
{
  if (player->movingUp && player->verticalPosition > (1 + PLAYER_HEIGHT / 2))
    {
...
resetGame ()
{
  ...
  if (playerRightScore > playerLeftScore)
    drawWinner (180, 50, whiteColor, framebuffer);
  else
    drawWinner (50, 50, whiteColor, framebuffer);
